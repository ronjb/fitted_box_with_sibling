...
// Copied and adapted from Flutter test/rendering/rendering_tester.dart
// ignore_for_file: parameter_assignments, cascade_invocations
...
class TestRenderingFlutterBinding extends BindingBase
  ...
  ///
  /// If [onErrors] is not null, it is called if [FlutterError] caught any
  /// errors while drawing the frame. If [onErrors] is null and [FlutterError]
  /// caught at least one error, this function fails the test. A test may
  /// override [onErrors] and inspect errors using [takeFlutterErrorDetails].
  ///
  ...
  void initInstances() {
    super.initInstances();
    _instance = this;
    // TO-DO(goderbauer): Create (fake) window if embedder doesn't provide an
    // implicit view.
  ...
  /// A function called after drawing a frame if [FlutterError] caught any
  /// errors.
  ///
  ...
  /// Returns all error details caught by [FlutterError] from least recently
  /// caught to most recently caught, and removes them from the list of
  /// captured errors.
  ///
  /// The returned iterable takes errors lazily. If, for example, you iterate
  /// over 2 errors, but there are 5 errors total, this binding will still fail
  /// the test. Tests are expected to take and inspect all errors.
  Iterable<FlutterErrorDetails> takeAllFlutterErrorDetails() sync* {
    // sync* and yield are used for lazy evaluation. Otherwise, the list would
    // be drained eagerly and allow a test pass with unexpected errors.
    while (_errors.isNotEmpty) {
      yield _errors.removeAt(0);
    }
  }

  /// Returns all exceptions caught by [FlutterError] from least recently
  /// caught to most recently caught, and removes them from the list of
  /// captured errors.
  ///
  /// The returned iterable takes errors lazily. If, for example, you iterate
      ...
      if (_errors.isNotEmpty) {
        if (onErrors != null) {
          ...
          if (_errors.isNotEmpty) {
            ...
            fail(
              'There are more errors than the test inspected using '
              'TestRenderingFlutterBinding.takeFlutterErrorDetails.',
            );
          }
        } else {
          _errors.forEach(FlutterError.dumpErrorToConsole);
          fail(
            'Caught error while rendering frame. See preceding '
            'logs for details.',
          );
        }
      }
  ...
  void drawFrame() {
    assert(
      ...
      'rendering_tester does not support testing the build phase; use '
      'flutter_test instead',
    );
    final oldErrorHandler = FlutterError.onError;
    FlutterError.onError = _errors.add;
    try {
...
void absorbOverflowedErrors() {
  ...
  final filtered = errorDetails.where((details) {
